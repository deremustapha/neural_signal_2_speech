...
FLAGS = flags.FLAGS
FLAGS([''])
flags.DEFINE_string('normalizers_file', '', 'file with pickled feature normalizers')
...
# phoneme_inventory = ['AA', 'AE', 'AH', 'AO', 'AW',
#     'AY', 'B',  'CH', 'D', 'DH',
#     'EH', 'ER', 'EY', 'F', 'G',
#     'HH', 'IH', 'IY', 'JH', 'K',
#     'L', 'M', 'N', 'NG', 'OW',
...
def decollate_tensor(tensor, lengths):
    ...
    for length in lengths:
        #print(f'idx {idx} length {length} b {b} s {s}')
        #print(f'Sum of lengths {idx + length }, {sum(lengths)} total size {b*s}')
